...
// camera 
Camera camera(glm::vec3(0.f, 0.0f, 4.30f), glm::vec3(0.0, 1.0, 0.0), -90.0);
...
int main(void)
{
	...
	Shader godsray(
		"shader/godsray/godsray.vs",
		"shader/godsray/godsray.fs");
	
	// render cube
	Shader cubeObject(
		"shader/5_section_shaders/3_11_lighting_2_out.vs",
		"shader/godsray/rendercube.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float x = sin(totalTime) * 3.0f;
		//float z = cos(totalTime) * 1.0f + 5.0f;
		lightPosition.x = x;
		//
		...
		cubeObject.SetVec3("lightColor", lightColor);
		cubeObject.SetVec3("LightPosition", lightPosition);
		...
		for (int y = -2; y < 3; y ++)
		{
			...
			{
				model = glm::mat4(1.0f);
				model = glm::translate(model, glm::vec3(float(x) * 0.5f, float(y)* 0.5f, 0.0));
		...
		lightObject.SetVec3("lightColor", lightColor);
		renderCube();
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		godsray.Use();
		godsray.SetMat4("projection", projection);
		godsray.SetMat4("view", view);
		godsray.SetBool("godsray", bloom);
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(lightPosition));
		godsray.SetMat4("model", model);
		glActiveTexture(GL_TEXTURE0);
