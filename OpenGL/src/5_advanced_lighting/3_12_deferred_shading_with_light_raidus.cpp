...
int main(void)
{
	...
	Shader lightGeometryPass(
		"shader/5_section_shaders/3_13_lighting_geomety_buffer_radius.vs",
		"shader/5_section_shaders/3_13_lighting_geomety_buffer_radius.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		else if (startIndex == 3)
		{
			...
			for (unsigned int i = 0; i < lightPositions.size(); i++)
			{
				...
				lightGeometryPass.SetFloat("lights[" + std::to_string(i) + "].Radius", radius);
			}
