...
int main(void)
{
	...
	Shader pbrShader(
		"shader/6_section_shaders/2_4_pbr_lighting_point_light_irradiance_map.vs",
		"shader/6_section_shaders/2_4_pbr_lighting_point_light_irradiance_map.fs");

	Shader pbrTextureShader(
		"shader/6_section_shaders/1_3_pbr_lighting_point_light_texture.vs",
		"shader/6_section_shaders/1_3_pbr_lighting_point_light_texture.fs");
	
	Shader equirectanglarToCubeMapShader(
		"shader/6_section_shaders/2_1_equrectangler_to_cube_map.vs",
		"shader/6_section_shaders/2_1_equrectangler_to_cube_map.fs");

	Shader cubeMapConvolutionShader(
		"shader/6_section_shaders/2_3_cube_map_to_irradiance_map.vs",
		"shader/6_section_shaders/2_3_cube_map_to_irradiance_map.fs");

	Shader skyBoxShader(
		"shader/6_section_shaders/2_2_skybox.vs",
		"shader/6_section_shaders/2_2_skybox.fs");
	...
	pbrTextureShader.SetFloat("aoMap", 4);
	...
	equirectanglarToCubeMapShader.Use();
	skyBoxShader.Use();
	skyBoxShader.SetInt("environmentMap", 0);

	//unsigned int albedoTex = loadTexture("Resources/textures/pbr/metalgrid/basecolor.png");
	//unsigned int normalTex = loadTexture("Resources/textures/pbr/metalgrid/normal.png");
	//unsigned int metallicTex = loadTexture("Resources/textures/pbr/metalgrid/metallic.png");
	//unsigned int roughnessTex = loadTexture("Resources/textures/pbr/metalgrid/roughness.png");
	//unsigned int aoTex = loadTexture("Resources/textures/pbr/metalgrid/ao.png");

	unsigned int albedoTex = loadTexture("Resources/textures/pbr/rustediron2/basecolor.png");
	unsigned int normalTex = loadTexture("Resources/textures/pbr/rustediron2/normal.png");
	unsigned int metallicTex = loadTexture("Resources/textures/pbr/rustediron2/metallic.png");
	unsigned int roughnessTex = loadTexture("Resources/textures/pbr/rustediron2/roughness.png");
	unsigned int aoTex = loadTexture("Resources/textures/pbr/rustediron2/ao.png");
	// lights
	// ------
	...
	// load HDR to environment map
	stbi_set_flip_vertically_on_load(true);
	int width, height, nrComponents;
//	float *data = stbi_loadf("Resources/textures/hdr/Newport_Loft/Newport_Loft_Ref.hdr", &width, &height, &nrComponents, 0);
	float *data = stbi_loadf("Resources/textures/hdr/Arches_E_PineTree/Arches_E_PineTree_3k.hdr", &width, &height, &nrComponents, 0);
	...
	equirectanglarToCubeMapShader.SetMat4("projection", captureProjection);
	...
	for (unsigned int i = 0; i < 6; ++i)
	{
		equirectanglarToCubeMapShader.SetMat4("view", captureViews[i]);
	...
	for (unsigned int i = 0; i < 6; ++i)
	{
		cubeMapConvolutionShader.SetMat4("view", captureViews[i]);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//equirectanglarToCubeMapShader.Use();
		//equirectanglarToCubeMapShader.SetMat4("view", view);
		//glActiveTexture(GL_TEXTURE0);
		//glBindTexture(GL_TEXTURE_2D, hdrTexture);
		//renderCube();
